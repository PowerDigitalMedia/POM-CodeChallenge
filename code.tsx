...
interface NameListItemProps {
    ...
    readonly name: string; readonly onNameClick: (clickedName: string) => void;

}

const NameListItem: React.FC <NameListItemProps> = (props: NameListItemProps) => (

    <li> <button onClick={() => props.onNameClick(props.name)}>{props.name}</button> </li>
); 
...
const NameList: React.FC = () => {
    ...
    const onNameClick = (clickedName: string) => {
        ...
        if(clickedName === undefined) 
        {
            ...
            alert("No State Change - No Re-Render");
        }//==
